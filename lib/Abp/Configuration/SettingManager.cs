...
namespace Abp.Configuration
{
    ...
    public class SettingManager : ISettingManager, ISingletonDependency
    {
        ...
        private readonly ITypedCache<
            string,
            Dictionary<string, SettingInfo>
        > _applicationSettingCache;
        ...
        )
        {
            ...
            SettingStore = NullConfigSettingStore.Instance;
        ...
        public Task<string> GetSettingValueForTenantAsync(
            string name,
            int tenantId,
            bool fallbackToDefault
        )
        {
            return GetSettingValueInternalAsync(
                name,
                tenantId,
        ...
        public Task<string> GetSettingValueForUserAsync(
            string name,
            int? tenantId,
            long userId,
            bool fallbackToDefault
        ...
        public string GetSettingValueForUser(
            string name,
            int? tenantId,
            long userId,
            bool fallbackToDefault
        ...
        {
            return await GetAllSettingValuesAsync(
        ...
        )
        {
            ...
            foreach (var setting in _settingDefinitionManager.GetAllSettingDefinitions())
            {
                ...
                settingValues[setting.Name] = new SettingValueObject(
                    setting.Name,
            ...
            if (scopes.HasFlag(SettingScopes.Application))
            {
                foreach (var settingValue in await GetAllSettingValuesForApplicationAsync())
                {
                    ...
                    if (
                        ...
                        && (
                            (
                                setting.Scopes.HasFlag(SettingScopes.Tenant)
                                && AbpSession.TenantId.HasValue
                            )
                            || (
                                setting.Scopes.HasFlag(SettingScopes.User)
                    ...
                    settingValues[settingValue.Name] = new SettingValueObject(
                        settingValue.Name,
                        settingValue.Value
                    );
                }
            ...
            {
                foreach (
                    var settingValue in await GetAllSettingValuesForTenantAsync(
            ...
            {
                foreach (
                    var settingValue in await GetAllSettingValuesForUserAsync(
        ...
        public IReadOnlyList<ISettingValue> GetAllSettingValues(SettingScopes scopes)
        {
            ...
            {
                foreach (
                    var settingValue in GetAllSettingValuesForTenant(AbpSession.TenantId.Value)
                )
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
            ...
            {
                foreach (
                    var settingValue in GetAllSettingValuesForUser(AbpSession.ToUserIdentifier())
                )
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
                    if (setting != null && setting.Scopes.HasFlag(SettingScopes.User))
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesForApplicationAsync()
        {
            ...
            {
                return (await GetReadOnlyTenantSettingsAsync(AbpSession.GetTenantId()))
                    .Values.Select(setting => new SettingValueObject(setting.Name, setting.Value))
                    .ToImmutableList();
            }

            return (await GetApplicationSettingsAsync())
                .Values.Select(setting => new SettingValueObject(setting.Name, setting.Value))
                .ToImmutableList();
        }
        ...
        public IReadOnlyList<ISettingValue> GetAllSettingValuesForApplication()
        {
            if (!_multiTenancyConfig.IsEnabled)
            {
                return (GetReadOnlyTenantSettings(AbpSession.GetTenantId()))
                    .Values.Select(setting => new SettingValueObject(setting.Name, setting.Value))
                    .ToImmutableList();
            }
        ...
        /// <inheritdoc/>
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesForTenantAsync(
            int tenantId
        )
        {
            return (await GetReadOnlyTenantSettingsAsync(tenantId))
                .Values.Select(setting => new SettingValueObject(setting.Name, setting.Value))
                .ToImmutableList();
        }
        ...
        {
            return (GetReadOnlyTenantSettings(tenantId))
                .Values.Select(setting => new SettingValueObject(setting.Name, setting.Value))
                .ToImmutableList();
        }
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesForUserAsync(
            UserIdentifier user
        )
        {
            return (await GetReadOnlyUserSettingsAsync(user))
                .Values.Select(setting => new SettingValueObject(setting.Name, setting.Value))
                .ToImmutableList();
        }
        ...
        {
            return (GetReadOnlyUserSettings(user))
                .Values.Select(setting => new SettingValueObject(setting.Name, setting.Value))
                .ToImmutableList();
        }
        ...
        public virtual async Task ChangeSettingForApplicationAsync(string name, string value)
        {
            await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                ...
                else
                {
                    ...
                    await InsertOrUpdateOrDeleteSettingValueAsync(
                        name,
                        value,
        ...
        /// <inheritdoc/>
        public virtual async Task ChangeSettingForTenantAsync(
            int tenantId,
            string name,
            string value
        )
        ...
        {
            return ChangeSettingForUserAsync(
                new UserIdentifier(AbpSession.TenantId, userId),
                name,
                value
            );
        }
        ...
        /// <inheritdoc/>
        public virtual async Task ChangeSettingForUserAsync(
            UserIdentifier user,
            string name,
            string value
        )
        ...
        private async Task<string> GetSettingValueInternalAsync(
            string name,
            int? tenantId = null,
            long? userId = null,
        ...
        private string GetSettingValueInternal(
            string name,
            int? tenantId = null,
            long? userId = null,
            bool fallbackToDefault = true
        )
        {
            ...
            if (settingDefinition.Scopes.HasFlag(SettingScopes.User) && userId.HasValue)
            {
                var settingValue = GetSettingValueForUserOrNull(
                    new UserIdentifier(tenantId, userId.Value),
                    name
                );
                if (settingValue != null)
        ...
        )
        {
            ...
            if (settingDefinition.IsInherited)
            {
                ...
                {
                    var tenantValue = await GetSettingValueForTenantOrNullAsync(
                        tenantId.Value,
                        name
                    );
                    if (tenantValue != null)
        ...
        private SettingInfo InsertOrUpdateOrDeleteSettingValue(
            string name,
            string value,
            int? tenantId,
            long? userId
        ...
        private async Task<SettingInfo> GetSettingValueForApplicationOrNullAsync(string name)
        {
            ...
            return (await GetReadOnlyTenantSettingsAsync(AbpSession.GetTenantId())).GetOrDefault(
                name
            );
        }
        ...
        private async Task<SettingInfo> GetSettingValueForTenantOrNullAsync(
            int tenantId,
            string name
        )
        {
        ...
        private async Task<SettingInfo> GetSettingValueForUserOrNullAsync(
            UserIdentifier user,
            string name
        )
        {
        ...
        {
            return await _applicationSettingCache.GetAsync(
        ...
        {
            return _applicationSettingCache.Get(
                ApplicationSettingsCacheKey,
                () =>
                {
        ...
        private async Task<ImmutableDictionary<string, SettingInfo>> GetReadOnlyTenantSettingsAsync(
            int tenantId
        )
        {
        ...
        private async Task<ImmutableDictionary<string, SettingInfo>> GetReadOnlyUserSettingsAsync(
            UserIdentifier user
        )
        {
        ...
        private ImmutableDictionary<string, SettingInfo> GetReadOnlyUserSettings(
            UserIdentifier user
        )
        {
        ...
        private async Task<Dictionary<string, SettingInfo>> GetTenantSettingsFromCacheAsync(
            int tenantId
        )
        {
        ...
        private async Task<Dictionary<string, SettingInfo>> GetUserSettingsFromCacheAsync(
            UserIdentifier user
        )
        {
            return await _userSettingCache.GetAsync(
                ...
                async () =>
                {
                    var settingValues = await SettingStore.GetAllListAsync(
                        user.TenantId,
                        user.UserId
                    );
                    return ConvertSettingInfosToDictionary(settingValues);
        ...
        private Dictionary<string, SettingInfo> ConvertSettingInfosToDictionary(
            List<SettingInfo> settingValues
        )
        {
            ...
            foreach (
                var setting in allSettingDefinitions.Join(
                    settingValues,
                    definition => definition.Name,
                    value => value.Name,
                    (definition, value) =>
                        new { SettingDefinition = definition, SettingValue = value }
                )
            )
            {
                if (setting.SettingDefinition.IsEncrypted)
                {
                    setting.SettingValue.Value = SettingEncryptionService.Decrypt(
