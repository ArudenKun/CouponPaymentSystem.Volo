...
namespace Abp.Webhooks
{
    public class DefaultWebhookPublisher : ApplicationService, IWebhookPublisher
    {
        ...
        {
            var subscriptions =
                await _webhookSubscriptionManager.GetAllSubscriptionsIfFeaturesGrantedAsync(
                    AbpSession.TenantId,
                    webhookName
                );
        ...
        {
            var subscriptions =
                await _webhookSubscriptionManager.GetAllSubscriptionsOfTenantsIfFeaturesGrantedAsync(
                    tenantIds,
                    webhookName
                );
            await PublishAsync(webhookName, data, subscriptions, sendExactSameData, headers);
        ...
        )
        {
            ...
            {
                var webhookInfo = await SaveAndGetWebhookAsync(
                    subscriptionGroupedByTenant.Key,
                    webhookName,
                    data
                );
                ...
                foreach (var webhookSubscription in subscriptionGroupedByTenant)
                {
                    ...
                    await _backgroundJobManager.EnqueueAsync<WebhookSenderJob, WebhookSenderArgs>(
                        new WebhookSenderArgs
                        {
        ...
        {
            var subscriptions = _webhookSubscriptionManager.GetAllSubscriptionsIfFeaturesGranted(
                AbpSession.TenantId,
                webhookName
            );
        ...
        {
            var subscriptions =
                _webhookSubscriptionManager.GetAllSubscriptionsOfTenantsIfFeaturesGranted(
                    tenantIds,
                    webhookName
                );
            Publish(webhookName, data, subscriptions, sendExactSameData, headers);
        ...
        )
        {
            ...
            foreach (var subscriptionGroupedByTenant in subscriptionsGroupedByTenant)
            {
                var webhookInfo = SaveAndGetWebhook(
                    subscriptionGroupedByTenant.Key,
                    webhookName,
                    data
                );
                ...
                foreach (var webhookSubscription in subscriptionGroupedByTenant)
                {
                    ...
                    _backgroundJobManager.Enqueue<WebhookSenderJob, WebhookSenderArgs>(
                        new WebhookSenderArgs
                        {
                            TenantId = webhookSubscription.TenantId,
                            WebhookEventId = webhookInfo.Id,
        ...
        )
        {
            var webhookInfo = new WebhookEvent
            {
                ...
                Data =
                    _webhooksConfiguration.JsonSerializerOptions != null
        ...
        protected virtual WebhookEvent SaveAndGetWebhook(
            int? tenantId,
            string webhookName,
            object data
        )
