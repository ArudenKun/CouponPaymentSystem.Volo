...
public static class AbpLoggerExtensions
{
    ...
    {
        if (
            exception is IHasErrorCode exceptionWithErrorCode
        ...
        if (
            exception is IHasErrorDetails exceptionWithErrorDetails
            && !exceptionWithErrorDetails.Details.IsNullOrWhiteSpace()
        )
        {
    ...
    private static void LogSelfLogging(ILogger logger, Exception exception)
    {
        ...
        if (exception is IExceptionWithSelfLogging logging)
        {
            ...
            exception is AggregateException aggException
            && aggException.InnerException != null
        )
        {
