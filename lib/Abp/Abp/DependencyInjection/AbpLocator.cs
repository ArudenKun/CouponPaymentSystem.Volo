...
public sealed class AbpLocator : IServiceProvider
{
    private AbpLocator() { }

    /// <summary>
    /// Gets the default <see cref="AbpLocator"/> instance.
    /// </summary>
    public static AbpLocator Instance { get; } = new();
    ...
    /// <exception cref="InvalidOperationException">Thrown if the current <see cref="AbpLocator"/> instance has not been initialized.</exception>
    ...
    /// Thrown if the current <see cref="AbpLocator"/> instance has not been initialized, or if the
    ...
    {
        IServiceProvider? oldServices = Interlocked.CompareExchange(
    ...
    private static void ThrowInvalidOperationExceptionForRepeatedConfiguration()
    {
        throw new InvalidOperationException(
            "The default service provider has already been configured."
        );
    }
}
