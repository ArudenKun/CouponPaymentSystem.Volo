...
public static class HasExtraPropertiesExtensions
{
    ...
    {
        return source.ExtraProperties?.ContainsKey(name) == true;
    }
    ...
    public static void SetDefaultsForExtraProperties(object source, Type objectType)
    {
        if (!(source is IHasExtraProperties))
        {
            throw new ArgumentException(
                $"Given {nameof(source)} object does not implement the {nameof(IHasExtraProperties)} interface!",
                nameof(source)
            );
        }
    ...
    public static void SetExtraPropertiesToRegularProperties(this IHasExtraProperties source)
    {
        var properties = source
            ...
            .GetProperties()
            .Where(x =>
                source.ExtraProperties?.ContainsKey(x.Name) == true && x.GetSetMethod(true) != null
            )
            .ToList();
    ...
    )
    {
        ...
        return source.ExtraProperties?.HasSameItems(
                other.ExtraProperties ?? new ExtraPropertyDictionary()
            ) == true;
    }
}
