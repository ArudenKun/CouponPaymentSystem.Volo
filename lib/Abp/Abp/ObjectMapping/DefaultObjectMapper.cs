...
{
    protected static ConcurrentDictionary<string, MethodInfo> MethodInfoCache { get; } = new();
    ...
    public virtual TDestination Map<TSource, TDestination>(TSource source)
    {
        ...
        using (var scope = ServiceProvider.CreateScope())
        {
            ...
            if (
                TryToMapCollection<TSource, TDestination>(
                    scope,
                    source,
                    default,
                    out var collectionResult
                )
            )
            {
    ...
    protected virtual bool TryToMapCollection<TSource, TDestination>(
    ...
    {
        if (
            !IsCollectionGenericType<TSource, TDestination>(
                out var sourceArgumentType,
                out var destinationArgumentType,
            ...
            _ =>
            {
                var methods = specificMapper
                    ...
                    .Where(x =>
                    {
                        ...
                        if (
                            destination == null && parameters.Length != 1
                        ...
                        return destination == null
                            || parameters[1].ParameterType == destinationArgumentType;
                    })
                    .ToList();
                ...
                if (methods.IsNullOrEmpty())
                {
                    throw new AbpException(
                        $"Could not find a method named '{nameof(IObjectMapper<object, object>.Map)}'"
                ...
                {
                    throw new AbpException(
                        $"Found more than one method named '{nameof(IObjectMapper<object, object>.Map)}'"
                            + $" with parameters({(destination == null ? sourceArgumentType.ToString() : sourceArgumentType + "," + destinationArgumentType)})"
                            + $" in the type '{mapperType}'."
                    );
                }
        ...
        var result = definitionGenericType.IsGenericType
            ? Activator
                .CreateInstance(definitionGenericType.MakeGenericType(destinationArgumentType))!
                .As<IList>()
        ...
        for (var i = 0; i < sourceList.Count; i++)
        {
            var invokeResult =
                destination == null
                    ...
                    : method.Invoke(
                        specificMapper,
                        new[] { sourceList[i], Activator.CreateInstance(destinationArgumentType)! }
                    )!;
    ...
    )
    {
        ...
        if (
            (!typeof(TSource).IsGenericType && !typeof(TSource).IsArray)
        ...
        if (
            typeof(TSource).IsGenericType
            && supportedCollectionTypes.Any(x => x == typeof(TSource).GetGenericTypeDefinition())
        )
        {
        ...
        if (
            typeof(TDestination).IsGenericType
            && supportedCollectionTypes.Any(x =>
                x == typeof(TDestination).GetGenericTypeDefinition()
            )
        )
        {
