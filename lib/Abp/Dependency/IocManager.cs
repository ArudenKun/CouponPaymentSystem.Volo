...
namespace Abp.Dependency
{
    ...
    public class IocManager : IIocManager
    {
        ...
        private List<ServiceDescriptor> WindsorDescriptors =>
            IocContainer
                .Kernel.GetHandlers()
                .SelectMany(h =>
                    h.ComponentModel.Services.Select(service =>
                    {
                        var impl = h.ComponentModel.Implementation;
                        var lifestyle = h.ComponentModel.LifestyleType switch
                        {
                            Castle.Core.LifestyleType.Singleton => ServiceLifetime.Singleton,
                            Castle.Core.LifestyleType.Scoped => ServiceLifetime.Scoped,
                            _ => ServiceLifetime.Transient,
                        };

                        return new ServiceDescriptor(service, impl, lifestyle);
                    })
                )
                .ToList();
        ...
        {
            var container = new WindsorContainer(new DefaultProxyFactory(ProxyGeneratorInstance));
        ...
        {
            IocContainer.Register(
                ApplyLifestyle(Component.For<TType, TImpl>().ImplementedBy<TImpl>(), lifeStyle)
            );
        }
        ...
        {
            IocContainer.Register(
                ApplyLifestyle(Component.For(type, impl).ImplementedBy(impl), lifeStyle)
            );
        }
        ...
        {
            return IocContainer
                .ResolveAll(type, Arguments.FromProperties(argumentsAsAnonymousType))
                .Cast<object>()
                .ToArray();
        }
        ...
        private static ComponentRegistration<T> ApplyLifestyle<T>(
        ...
        public IEnumerator<ServiceDescriptor> GetEnumerator() => WindsorDescriptors.GetEnumerator();

        IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();

        public void Add(ServiceDescriptor item) => IocContainer.Add(item);

        public void Clear() => throw new NotSupportedException();

        public bool Contains(ServiceDescriptor item) => WindsorDescriptors.Contains(item);

        public void CopyTo(ServiceDescriptor[] array, int arrayIndex) =>
            WindsorDescriptors.CopyTo(array, arrayIndex);

        public bool Remove(ServiceDescriptor item) => throw new NotSupportedException();

        public int Count => WindsorDescriptors.Count;
        public bool IsReadOnly => false;

        public int IndexOf(ServiceDescriptor item) => WindsorDescriptors.IndexOf(item);
