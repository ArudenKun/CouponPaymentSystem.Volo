...
namespace Abp.Localization.Dictionaries.Json
{
    ...
    public class JsonEmbeddedFileLocalizationDictionaryProvider : LocalizationDictionaryProviderBase
    {
        ...
        /// </para>
        /// </param>
        public JsonEmbeddedFileLocalizationDictionaryProvider(
            Assembly assembly,
            ...
            var resourceNames = _assembly
                ...
                .Where(resouceName =>
                    allCultureInfos.Any(culture =>
                        resouceName.EndsWith($"{SourceName}.json", true, null)
                        || resouceName.EndsWith($"{SourceName}-{culture.Name}.json", true, null)
                    )
                )
                .ToList();
                ...
                if (resourceName.StartsWith(_rootNamespace))
                {
                    using (var stream = _assembly.GetManifestResourceStream(resourceName))
                    {
                        ...
                        InitializeDictionary(
                            CreateJsonLocalizationDictionary(jsonString),
