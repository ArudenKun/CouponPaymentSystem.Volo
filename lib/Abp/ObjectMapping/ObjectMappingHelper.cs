...
public static class ObjectMappingHelper
{
    private static readonly ConcurrentDictionary<
        (Type, Type),
        (Type sourceArgumentType, Type destinationArgumentType, Type definitionGenericType)?
    > Cache = new();
    ...
    )
    {
        var cached = Cache.GetOrAdd(
            ...
            _ => IsCollectionGenericTypeInternal<TSource, TDestination>()
        );
        if (cached == null)
        {
    ...
    {
        if (
            !IsCollectionGenericTypeInternal(typeof(TSource), out var sourceArgumentType, out _)
            || !IsCollectionGenericTypeInternal(
                typeof(TDestination),
    ...
    private static bool IsCollectionGenericTypeInternal(
        Type type,
    ...
    )
    {
        ...
        if (
            type.IsGenericType && supportedCollectionTypes.Contains(type.GetGenericTypeDefinition())
            || type.GetInterfaces()
                .Any(i =>
                    i.IsGenericType
                    && supportedCollectionTypes.Contains(i.GetGenericTypeDefinition())
                )
        )
        {
            ...
            if (
                definitionGenericType == typeof(IEnumerable<>)
