...
public static class ObjectHelper
{
    ...
    )
    {
        ...
        var property = CachedObjectProperties.GetOrAdd(cacheKey, PropertyFactory);
        ...
        PropertyInfo? PropertyFactory(string _)
        {
            ...
            switch (propertySelector.Body.NodeType)
            {
                ...
                {
                    memberExpression =
                        propertySelector.Body.As<UnaryExpression>().Operand as MemberExpression;
                    break;
                }
            ...
            if (
                ...
                && ignoreAttributeTypes.Any(ignoreAttribute =>
                    propertyInfo.IsDefined(ignoreAttribute, true)
                )
            )
            {
                return null;
