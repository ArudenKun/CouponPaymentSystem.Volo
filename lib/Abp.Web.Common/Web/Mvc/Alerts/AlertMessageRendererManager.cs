...
namespace Abp.Web.Mvc.Alerts
{
    public class AlertMessageRendererManager : IAlertMessageRendererManager
    {
        ...
        public string Render(string alertDisplayType)
        {
            ...
            var alertMessageRenderer = _alertMessageRenderers.FirstOrDefault(x =>
                x.DisplayType == alertDisplayType
            );

            return alertMessageRenderer?.Render(
                _alertManager.Alerts.Where(a => a.DisplayType == alertDisplayType).ToList()
            );
        }
    }
}
