...
namespace Abp.Web.Timing
{
    ...
    public class TimingScriptManager : ITimingScriptManager, ITransientDependency
    {
        ...
        public async Task<string> GetScriptAsync()
        {
            ...
            script.AppendLine(
                "    abp.clock.provider = abp.timing."
                    + Clock.Provider.GetType().Name.ToCamelCase()
            ...
            {
                script.AppendLine(
                    "    abp.timing.timeZoneInfo = " + await GetUsersTimezoneScriptsAsync()
                );
            }
        ...
        {
            var timezoneId = await _settingManager.GetSettingValueAsync(
            ...
            return " {"
                ...
                + timezone.BaseUtcOffset.TotalMilliseconds
                + "',"
                + "            currentUtcOffsetInMilliseconds: '"
                + timezone.GetUtcOffset(Clock.Now).TotalMilliseconds
                + "',"
                + "            isDaylightSavingTimeNow: '"
                + timezone.IsDaylightSavingTime(Clock.Now)
                + "'"
                + "        },"
                + "        iana: {"
                + "            timeZoneId:'"
                + TimezoneHelper.WindowsToIana(timezoneId)
                + "'"
                + "        },"
                + "    }";
        }
