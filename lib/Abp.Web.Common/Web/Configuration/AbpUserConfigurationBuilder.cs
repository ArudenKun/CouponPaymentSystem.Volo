    ...
    public class AbpUserConfigurationBuilder : ITransientDependency
    {
        ...
        protected virtual AbpUserLocalizationConfigDto GetUserLocalizationConfig()
        {
            ...
            config.Sources = sources
                .Select(s => new AbpLocalizationSourceDto
                {
                    Name = s.Name,
                    Type = s.GetType().Name,
                })
            ...
            {
                var stringValues = source
                    .GetAllStrings(currentCulture)
                    .OrderBy(s => s.Name)
                    .ToList();
                var stringDictionary = stringValues.ToDictionary(_ => _.Name, _ => _.Value);
        ...
        protected virtual async Task<AbpUserFeatureConfigDto> GetUserFeaturesConfig()
        {
            ...
            if (AbpSession.TenantId.HasValue)
            {
                ...
                foreach (var feature in allFeatures)
                {
                    ...
                    config.AllFeatures.Add(feature.Name, new AbpStringValueDto { Value = value });
                }
            }
            else
            {
        ...
        protected virtual async Task<AbpUserAuthConfigDto> GetUserAuthConfig()
        {
            ...
            var allPermissionNames = PermissionManager
            ...
            config.AllPermissions = allPermissionNames.ToDictionary(
                permissionName => permissionName,
                permissionName => "true"
            );
            config.GrantedPermissions = grantedPermissionNames.ToDictionary(
                permissionName => permissionName,
                permissionName => "true"
            );
        ...
        {
            var userMenus = await UserNavigationManager.GetMenusAsync(
        ...
        {
            var config = new AbpUserSettingConfigDto { Values = new Dictionary<string, string>() };
            ...
            using (var scope = _iocResolver.CreateScope())
            {
                ...
                {
                    if (
                        !await SettingDefinitionManager
                            .GetSettingDefinition(settingValue.Name)
                            .ClientVisibilityProvider.CheckVisible(scope)
                    )
                    {
                        continue;
                    }
            ...
            return new AbpUserTimingConfigDto
            {
                TimeZoneInfo = new AbpUserTimeZoneConfigDto
                {
                    Windows = new AbpUserWindowsTimeZoneConfigDto
                    {
                        ...
                        CurrentUtcOffsetInMilliseconds = timezone
