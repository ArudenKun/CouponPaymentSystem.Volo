...
namespace Abp.Web.Localization
{
    internal class LocalizationScriptManager : ILocalizationScriptManager, ISingletonDependency
    {
        ...
        private string BuildAll(CultureInfo cultureInfo)
        {
            ...
            script.AppendLine(
                "        name: '" + HttpEncode.JavaScriptStringEncode(cultureInfo.Name) + "',"
            );
            script.AppendLine(
                "        displayName: '"
                    + HttpEncode.JavaScriptStringEncode(cultureInfo.DisplayName)
                    + "'"
            );
            script.AppendLine("    };");
            script.AppendLine();
            ...
            for (var i = 0; i < languages.Count; i++)
            {
                ...
                script.AppendLine("{");
                script.AppendLine(
                    "        name: '" + HttpEncode.JavaScriptStringEncode(language.Name) + "',"
                );
                script.AppendLine(
                    "        displayName: '"
                        + HttpEncode.JavaScriptStringEncode(language.DisplayName)
                        + "',"
                );
                script.AppendLine(
                    "        icon: '" + HttpEncode.JavaScriptStringEncode(language.Icon) + "',"
                );
                script.AppendLine(
                    "        isDisabled: " + language.IsDisabled.ToString().ToLowerInvariant() + ","
                );
                script.AppendLine(
                    "        isDefault: " + language.IsDefault.ToString().ToLowerInvariant()
                );
                script.Append("    }");
            ...
            if (languages.Count > 0)
            {
                ...
                script.AppendLine(
                    "        name: '"
                        + HttpEncode.JavaScriptStringEncode(currentLanguage.Name)
                        + "',"
                );
                script.AppendLine(
                    "        displayName: '"
                ...
                script.AppendLine(
                    "        icon: '"
                        + HttpEncode.JavaScriptStringEncode(currentLanguage.Icon)
                        + "',"
                );
                script.AppendLine(
                    "        isDisabled: "
                        + currentLanguage.IsDisabled.ToString().ToLowerInvariant()
                        + ","
                );
                script.AppendLine(
                    "        isDefault: " + currentLanguage.IsDefault.ToString().ToLowerInvariant()
                );
                script.AppendLine("    };");
            }
            ...
            for (int i = 0; i < sources.Length; i++)
            {
                ...
                script.AppendLine("        {");
                script.AppendLine(
                    "            name: '" + HttpEncode.JavaScriptStringEncode(source.Name) + "',"
                );
            ...
            foreach (var source in sources)
            {
                script.Append(
                    "    abp.localization.values['"
                        + HttpEncode.JavaScriptStringEncode(source.Name)
                        + "'] = "
                );
