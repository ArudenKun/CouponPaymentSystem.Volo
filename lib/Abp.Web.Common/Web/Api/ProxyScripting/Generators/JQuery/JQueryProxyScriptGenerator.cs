...
namespace Abp.Web.Api.ProxyScripting.Generators.JQuery
{
    public class JQueryProxyScriptGenerator : IProxyScriptGenerator, ITransientDependency
    {
        ...
        private static void AddModuleScript(StringBuilder script, ModuleApiDescriptionModel module)
        {
            ...
            script.AppendLine();
            script.AppendLine(
                $"  abp.services.{module.Name.ToCamelCase()} = abp.services.{module.Name.ToCamelCase()} || {{}};"
            );
        ...
        )
        {
            ...
            script.AppendLine(
                $"    abp.services.{module.Name.ToCamelCase()}.{controller.Name.ToCamelCase()} = abp.services.{module.Name.ToCamelCase()}.{controller.Name.ToCamelCase()} || {{}};"
            );
        ...
        private static void AddActionScript(
            StringBuilder script,
            ModuleApiDescriptionModel module,
            ControllerApiDescriptionModel controller,
        ...
        {
            var parameterList = ProxyScriptingJsFuncHelper.GenerateJsFuncParameterList(
                action,
                "ajaxParams"
            );
            ...
            script.AppendLine(
                $"    abp.services.{module.Name.ToCamelCase()}.{controller.Name.ToCamelCase()}{ProxyScriptingJsFuncHelper.WrapWithBracketsOrWithDotPrefix(action.Name.ToCamelCase())} = function({parameterList}) {{"
            );
        ...
        private static void AddAjaxCallParameters(
            StringBuilder script,
            ControllerApiDescriptionModel controller,
            ActionApiDescriptionModel action
        )
