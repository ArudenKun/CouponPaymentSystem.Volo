...
namespace Abp.Web.Navigation
{
    internal class NavigationScriptManager : INavigationScriptManager, ITransientDependency
    {
        ...
        {
            var userMenus = await _userNavigationManager.GetMenusAsync(
        ...
        private static void AppendMenu(StringBuilder script, UserMenu menu)
        {
            ...
            script.AppendLine(
                "            name: '" + HttpEncode.JavaScriptStringEncode(menu.Name) + "',"
            );
            ...
            if (menu.DisplayName != null)
            {
                script.AppendLine(
                    "            displayName: '"
                        + HttpEncode.JavaScriptStringEncode(menu.DisplayName)
                        + "',"
                );
            }
            ...
            {
                script.AppendLine(
                    "            customData: " + menu.CustomData.ToJsonString(true) + ","
                );
            }
