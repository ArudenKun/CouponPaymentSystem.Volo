        ...
        IServiceProviderIsService
{
    ...
    {
        get => InternalIsInResolving.Value;
        set => InternalIsInResolving.Value = value;
    }

    private static readonly AsyncLocal<bool> InternalIsInResolving = new();

    public ScopedWindsorServiceProvider(
    ...
    public object GetRequiredService(Type serviceType)
    {
        return GetServiceInternal(serviceType, false)
            ?? throw new InvalidOperationException("Failed to resolve service for " + serviceType);
    }

    private object? GetServiceInternal(Type serviceType, bool isOptional)
    {
        if (OwnMsLifetimeScope is null)
        {
            throw new InvalidOperationException("OwnMsLifetimeScope is null");
        }
    ...
    public bool IsService(Type serviceType)
    {
        ...
        )
        {
            ...
            return genericDefinition == typeof(IEnumerable<>)
                || _container.Kernel.HasComponent(genericDefinition);
        }
