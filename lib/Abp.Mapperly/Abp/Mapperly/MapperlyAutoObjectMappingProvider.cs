...
public class MapperlyAutoObjectMappingProvider<TContext>
...
public class MapperlyAutoObjectMappingProvider : IAutoObjectMappingProvider
{
    ...
    > MapCache = new();
    protected static readonly List<MethodInfo> MapMethods =
        typeof(MapperlyAutoObjectMappingProvider)
    ...
    public virtual TDestination Map<TSource, TDestination>(object source)
    {
        ...
        if (mapper != null)
        {
            ...
            TryMapExtraProperties(
                ...
                (TSource)source,
                destination,
                GetExtraProperties(destination)
            );
        ...
        var reverseMapper = ServiceProvider.GetService<
            IAbpReverseMapperlyMapper<TDestination, TSource>
        >();
    ...
    {
        if (
            !ObjectMappingHelper.IsCollectionGenericType<TSource, TDestination>(
        ...
        var mapperType = typeof(IAbpMapperlyMapper<,>).MakeGenericType(
            sourceArgumentType,
        ...
        if (mapper == null)
        {
            mapperType = typeof(IAbpReverseMapperlyMapper<,>).MakeGenericType(
                destinationArgumentType,
                sourceArgumentType
            );
    ...
    )
    {
        var method = !hasDestination
            ? MapMethods
                ...
                .MakeGenericMethod(sourceArgumentType, destinationArgumentType)
            : MapMethods
    ...
    protected virtual ExtraPropertyDictionary GetExtraProperties<TDestination>(
        TDestination destination
    )
    {
    ...
    )
    {
        ...
        if (
            sourceHasExtraProperties.ExtraProperties != null
            && sourceHasExtraProperties.ExtraProperties
