...
namespace Abp.Web.MultiTenancy
{
    public class DomainTenantResolveContributor : ITenantResolveContributor, ITransientDependency
    {
        ...
        public int? ResolveTenantId()
        {
            ...
            var hostName = httpContext
                .Request.Url.Host.RemovePreFix("http://", "https://")
                .RemovePostFix("/");
        ...
        private FormattedStringValueExtracter.ExtractionResult IsDomainFormatValid(
            string[] domainFormats,
            string hostName
        )
        {
            foreach (var item in domainFormats)
            {
                var domainFormat = item.RemovePreFix("http://", "https://")
                    .Split(':')[0]
                    .RemovePostFix("/");
                var result = new FormattedStringValueExtracter().Extract(
                    hostName,
                    domainFormat,
                    true,
                    '/'
