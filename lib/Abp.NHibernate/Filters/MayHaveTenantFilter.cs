...
public class MayHaveTenantFilter : FilterDefinition
{
    ...
    public MayHaveTenantFilter()
    {
        WithName(AbpDataFilters.MayHaveTenant)
            ...
            .WithCondition(
                $"((:{AbpDataFilters.Parameters.TenantId} IS NULL AND {nameof(IMayHaveTenant.TenantId)} IS NULL) OR ({nameof(IMayHaveTenant.TenantId)} = :{AbpDataFilters.Parameters.TenantId}))"
            );
    }
}
