...
internal class AbpNHibernateInterceptor : EmptyInterceptor
{
    ...
    public AbpNHibernateInterceptor(IIocManager iocManager)
    {
        ...
        _abpSession = new Lazy<IAbpSession>(
            () =>
        ...
        _guidGenerator = new Lazy<IGuidGenerator>(
            () =>
    ...
    )
    {
        ...
        if (entity is ICreationAudited && _abpSession.Value.UserId.HasValue)
        {
            for (var i = 0; i < propertyNames.Length; i++)
            {
                ...
                {
                    state[i] = (entity as ICreationAudited).CreatorUserId = _abpSession
                        .Value
                        .UserId;
                }
            }
    ...
    )
    {
        ...
        if (entity is IHasModificationTime)
        {
            for (var i = 0; i < propertyNames.Length; i++)
            {
                ...
                {
                    currentState[i] = (entity as IHasModificationTime).LastModificationTime =
                        Clock.Now;
                    updated = true;
                }
            }
        ...
        if (entity is IModificationAudited && _abpSession.Value.UserId.HasValue)
        {
            for (var i = 0; i < propertyNames.Length; i++)
            {
                ...
                {
                    currentState[i] = (entity as IModificationAudited).LastModifierUserId =
                        _abpSession.Value.UserId;
                    updated = true;
                }
            }
        }
    ...
    private static void NormalizeDateTimePropertiesForEntity(
        object entity,
        object[] state,
        string[] propertyNames,
        IList<IType> types
    )
    {
        for (var i = 0; i < types.Count; i++)
        {
            ...
            if (
                types[i].ReturnedClass != typeof(DateTime)
                && types[i].ReturnedClass != typeof(DateTime?)
            )
            {
                continue;
    ...
    private static void NormalizeDateTimePropertiesForComponentType(
    ...
    )
    {
        ...
        for (int i = 0; i < componentType.PropertyNames.Length; i++)
        {
            ...
            if (
                componentType.Subtypes[i].ReturnedClass != typeof(DateTime)
                && componentType.Subtypes[i].ReturnedClass != typeof(DateTime?)
            )
            {
                continue;
