...
/// </summary>
internal class NhUnitOfWork : UnitOfWorkBase, ITransientDependency
{
    /// <summary>
        ...
        if (Options.IsTransactional == true)
        {
            _transaction = Options.IsolationLevel.HasValue
                ? Session.BeginTransaction(
                    Options.IsolationLevel.Value.ToSystemDataIsolationLevel()
                )
                : Session.BeginTransaction();
        }
    ...
    protected virtual void CheckAndSetSoftDelete()
    {
        if (IsFilterEnabled(AbpDataFilters.SoftDelete))
        {
            ...
            ApplyFilterParameterValue(
                ...
                AbpDataFilters.Parameters.IsDeleted,
                false
            ); //ApplyFilter
            SetFilterParameter(
                AbpDataFilters.SoftDelete,
