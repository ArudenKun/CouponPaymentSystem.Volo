...
public static class WindsorRegistrationHelper
{
    ...
    internal static void AddServicesInternal(
        this IWindsorContainer container,
        IServiceCollection services
    )
    {
    ...
    private static void AddBaseServices(IWindsorContainer container)
    {
        ...
        {
            container.Register(Component.For<GlobalMsLifetimeScope>().LifestyleSingleton());
        }

        if (!container.Kernel.HasComponent(typeof(MsLifetimeScopeProvider)))
        {
            container.Register(Component.For<MsLifetimeScopeProvider>().LifestyleTransient());
        }
        ...
        if (!container.Kernel.HasComponent(typeof(IServiceProvider)))
        {
            container.Register(
                Component
                    .For<IServiceProvider, IServiceProviderIsService>()
        ...
        {
            container.Register(
                Component.For<GlobalScopedWindsorServiceProvider>().LifestyleTransient()
            );
        }
    }
    ...
    private static void AddSubResolvers(IWindsorContainer container)
    {
        ...
        container.Kernel.Resolver.AddSubResolver(
            new MsCompatibleCollectionResolver(container.Kernel)
        );
    ...
    private static void AddServicesCollection(
        IWindsorContainer container,
        IServiceCollection services
    )
    {
    ...
    public static void Add(this IWindsorContainer container, ServiceDescriptor serviceDescriptor) =>
        RegisterServiceDescriptor(container, serviceDescriptor);

    private static void RegisterServiceDescriptor(
        IWindsorContainer container,
    ...
    )
    {
        ...
        string uniqueName =
            serviceDescriptor.ServiceType.FullName + "_" + GuidPolyfill.CreateVersion7();
        ...
        if (serviceDescriptor.ImplementationType != null)
        {
            container.Register(
                Component
                    .For(serviceDescriptor.ServiceType)
                    .Named(uniqueName)
                    .IsDefault()
            ...
            container.Register(
                Component
                    ...
                    .UsingFactoryMethod(c =>
                        serviceDescriptorRef.ImplementationFactory(c.Resolve<IServiceProvider>())
                    )
                    .ConfigureLifecycle(serviceDescriptor.Lifetime)
            );
        }
