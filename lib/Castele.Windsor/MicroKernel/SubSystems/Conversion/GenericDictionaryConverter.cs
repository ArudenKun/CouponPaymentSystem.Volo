...
namespace Castle.MicroKernel.SubSystems.Conversion
{
    ...
    public class GenericDictionaryConverter : AbstractTypeConverter
    {
        ...
        public override object PerformConversion(IConfiguration configuration, Type targetType)
        {
            ...
            var helperType = typeof(DictionaryHelper<,>).MakeGenericType(
                defaultKeyType,
                defaultValueType
            );
            var collectionConverterHelper =
                helperType.CreateInstance<IGenericCollectionConverterHelper>(this);
                ...
                foreach (var itemConfig in configuration.Children)
                {
                    ...
                    if (keyValue == null)
                    {
                        throw new ConverterException(
                            "You must provide a key for the dictionary entry"
                        );
                    }
                    ...
                    {
                        convertKeyTo = parent.Context.Composition.PerformConversion<Type>(
                            itemConfig.Attributes["keyType"]
                        );
                    }
                    ...
                    if (convertKeyTo.Is<TKey>() == false)
                    {
                        throw new ArgumentException(
                            string.Format(
                                "Could not create dictionary<{0},{1}> because {2} is not assignable to key type {0}",
                                typeof(TKey),
                                typeof(TValue),
                                convertKeyTo
                            )
                    ...
                    var key = (TKey)
                        parent.Context.Composition.PerformConversion(keyValue, convertKeyTo);
                    ...
                    {
                        convertValueTo = parent.Context.Composition.PerformConversion<Type>(
                            itemConfig.Attributes["valueType"]
                        );
                    }
                    ...
                    if (convertValueTo.Is<TValue>() == false)
                    {
                        throw new ArgumentException(
                            string.Format(
                                "Could not create dictionary<{0},{1}> because {2} is not assignable to value type {1}",
                                typeof(TKey),
                                typeof(TValue),
                                convertValueTo
                            )
                    ...
                    if (itemConfig.Children.Count == 1)
                    {
                        dict.Add(
                            key,
                            (TValue)
                                parent.Context.Composition.PerformConversion(
                                    itemConfig.Children[0],
                                    convertValueTo
                                )
                        );
                    }
