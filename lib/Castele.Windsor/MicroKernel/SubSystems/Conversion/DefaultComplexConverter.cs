    ...
    public class DefaultComplexConverter : AbstractTypeConverter
    {
            ...
            if (String.IsNullOrEmpty(typeNode))
            {
                if (type.GetTypeInfo().IsInterface)
                {
                    throw new ConverterException(
                        "A type attribute must be specified for interfaces"
                    );
                }
        ...
        private ConstructorInfo ChooseConstructor(Type type)
        {
            ...
            foreach (var candidate in constructors)
            {
                ...
                {
                    throw new ConverterException(
                        "Classes with more than one non-default constructor are not supported."
                    );
                }
                chosen = candidate;
            }
        ...
        /// <param name = "configuration">The configuration.</param>
        /// <returns></returns>
        private object[] ConvertConstructorParameters(
