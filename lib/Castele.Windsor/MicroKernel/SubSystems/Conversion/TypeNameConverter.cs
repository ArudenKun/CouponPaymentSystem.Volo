...
namespace Castle.MicroKernel.SubSystems.Conversion
{
    ...
    public class TypeNameConverter : AbstractTypeConverter
    {
        ...
        private readonly IDictionary<string, MultiType> fullName2Type = new Dictionary<
            string,
            MultiType
        >(StringComparer.OrdinalIgnoreCase);

        private readonly IDictionary<string, MultiType> justName2Type = new Dictionary<
            string,
            MultiType
        >(StringComparer.OrdinalIgnoreCase);
        ...
        public override object PerformConversion(String value, Type targetType)
        {
            ...
            catch (Exception ex)
            {
                throw new ConverterException(
                    String.Format("Could not convert string '{0}' to a type.", value),
                    ex
                );
            }
        }
        ...
        private Type GetType(string name)
        {
            ...
            if (typeName == null)
            {
                throw new ConverterException(
                    String.Format(
                        "Could not convert string '{0}' to a type. It doesn't appear to be a valid type name.",
                        name
                    )
                );
            }
            ...
            if (assemblyName != null)
            {
                ...
                var assembly = assemblies.FirstOrDefault(a =>
                    a.FullName.StartsWith(namePart, StringComparison.OrdinalIgnoreCase)
                );
                if (assembly != null)
                {
                    throw new ConverterException(
        ...
        private bool InitializeAppDomainAssemblies(bool forceLoad)
        {
            ...
            if (assemblies.Count == 0)
            {
                ...
                var dependencies = context
                    .RuntimeLibraries.SelectMany(library =>
        ...
        private Type GetUniqueType(
            string name,
            IDictionary<string, MultiType> map,
            string description
        )
        {
            MultiType type;
        ...
        private void EnsureUnique(MultiType type, string value, string missingInformation)
        {
            ...
            var message = new StringBuilder(
                string.Format("Could not uniquely identify type for '{0}'. ", value)
            );
            ...
            message.AppendFormat(
                "Provide more information ({0}) to uniquely identify the type.",
                missingInformation
            );
