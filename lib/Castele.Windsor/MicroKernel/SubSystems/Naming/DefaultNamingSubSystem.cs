...
namespace Castle.MicroKernel.SubSystems.Naming
{
    ...
    public class DefaultNamingSubSystem : AbstractSubSystem, INamingSubSystem
    {
        ...
        /// </summary>
        protected readonly Dictionary<string, IHandler> name2Handler = new Dictionary<
            string,
            IHandler
        ...
        /// </summary>
        protected readonly Dictionary<Type, HandlerWithPriority> service2Handler = new Dictionary<
            Type,
            HandlerWithPriority
        >(SimpleTypeEqualityComparer.Instance);
        ...
        protected readonly IDictionary<Type, IHandler[]> handlerListsByTypeCache = new Dictionary<
            Type,
            IHandler[]
        >(SimpleTypeEqualityComparer.Instance);
        ...
        protected IDictionary<Type, IHandler> HandlerByServiceCache
        {
            get
            {
                ...
                using (@lock.ForWriting())
                {
                    cache = new Dictionary<Type, IHandler>(
                        service2Handler.Count,
                        service2Handler.Comparer
                    );
        ...
        public virtual IHandler GetHandler(Type service)
        {
            ...
            )
            {
                ...
                if (
                    HandlerByServiceCache.TryGetValue(openService, out handler)
        ...
        public virtual void Register(IHandler handler)
        {
            ...
            using (@lock.ForWriting())
            {
                ...
                foreach (var service in handler.ComponentModel.Services)
                {
                    ...
                    if (
                        service2Handler.TryGetValue(service, out previous) == false
        ...
        private Func<Type, HandlerWithPriority> GetServiceSelector(IHandler handler)
        {
            ...
            if (defaultsFilter == null)
            {
                ...
                return service => new HandlerWithPriority(
                    fallbackFilter(service) ? -1 : 0,
                    handler
                );
            }
            if (fallbackFilter == null)
            ...
            return service => new HandlerWithPriority(
                defaultsFilter(service) ? 1 : (fallbackFilter(service) ? -1 : 0),
                handler
            );
        }
