...
namespace Castle.MicroKernel.Registration.Lifestyle
{
        ...
        where TService : class
    {
        ...
        public ComponentRegistration<TService> Transient
        {
            get
            {
                return AddDescriptor(new LifestyleDescriptor<TService>(LifestyleType.Transient));
            }
        }
        ...
        public ComponentRegistration<TService> Singleton
        {
            get
            {
                return AddDescriptor(new LifestyleDescriptor<TService>(LifestyleType.Singleton));
            }
        }
        ...
        /// <returns> </returns>
        public ComponentRegistration<TService> Scoped<TScopeAccessor>()
        ...
        {
            var registration = AddDescriptor(
                new LifestyleDescriptor<TService>(LifestyleType.Scoped)
            );
            if (scopeAccessorType == null)
            {
                return registration;
            }
            var scopeAccessor = new ExtendedPropertiesDescriptor(
                new Property(Constants.ScopeAccessorType, scopeAccessorType)
            );
        ...
        public ComponentRegistration<TService> Custom(Type customLifestyleType)
        {
            if (customLifestyleType.Is<ILifestyleManager>() == false)
            {
                throw new ComponentRegistrationException(
                    String.Format(
                        "The type {0} must implement {1} to " + "be used as a custom lifestyle",
                        customLifestyleType.FullName,
            ...
            return AddDescriptor(new LifestyleDescriptor<TService>(LifestyleType.Custom))
                .Attribute("customLifestyleType")
                .Eq(customLifestyleType.AssemblyQualifiedName);
        }
