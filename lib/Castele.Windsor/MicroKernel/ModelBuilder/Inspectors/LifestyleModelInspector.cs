    ...
    public class LifestyleModelInspector : IContributeComponentModelConstruction
    {
            ...
            if (lifestyleRaw != null)
            {
                ...
                switch (lifestyleType)
                {
                    ...
                    case LifestyleType.Custom:
                        var lifestyle = GetMandatoryTypeFromAttribute(
                            model,
                            "customLifestyleType",
                            lifestyleType
                        );
                        ValidateTypeFromAttribute(
                            lifestyle,
                            typeof(ILifestyleManager),
                            "customLifestyleType"
                        );
                    ...
                    case LifestyleType.Scoped:
                        ...
                        if (scopeAccessorType != null)
                        {
                            ValidateTypeFromAttribute(
                                scopeAccessorType,
                                typeof(IScopeAccessor),
                                "scopeAccessorType"
                            );
                            model.ExtendedProperties[Constants.ScopeAccessorType] =
                                scopeAccessorType;
                        }
                        return true;
                    ...
                    default:
                        throw new InvalidOperationException(
                            string.Format(
                                "Component {0} has {1} lifestyle. This is not a valid value.",
                                model.Name,
                                lifestyleType
                            )
                        );
        ...
        protected virtual void ReadLifestyleFromType(ComponentModel model)
        {
            ...
            else if (model.LifestyleType == LifestyleType.Pooled)
            {
                ...
                model.ExtendedProperties[ExtendedPropertiesConstants.Pool_InitialPoolSize] =
                    pooled.InitialPoolSize;
                model.ExtendedProperties[ExtendedPropertiesConstants.Pool_MaxPoolSize] =
                    pooled.MaxPoolSize;
            }
            else if (model.LifestyleType == LifestyleType.Bound)
            {
                var binder = ExtractBinder(
                    ((BoundToAttribute)attribute).ScopeRootBinderType,
                    model.Name
                );
                model.ExtendedProperties[Constants.ScopeRootSelector] = binder;
            }
        ...
        protected virtual void ValidateTypeFromAttribute(
            Type typeFromAttribute,
        ...
        )
        {
            ...
            throw new InvalidOperationException(
                String.Format(
                    "The Type '{0}' specified in the '{2}' attribute must implement {1}",
                    typeFromAttribute.FullName,
        ...
        {
            var filterMethod = scopeRootBinderType
                .GetTypeInfo()
                .FindMembers(
                    MemberTypes.Method,
            ...
            return (Func<IHandler[], IHandler>)
                methodInfo.CreateDelegate(typeof(Func<IHandler[], IHandler>), instance);
        }
        ...
        private Type GetMandatoryTypeFromAttribute(
        ...
        )
        {
            ...
            if (rawAttribute == null)
            {
                throw new InvalidOperationException(
                    string.Format(
                        "Component {0} has {1} lifestyle, but its configuration doesn't have mandatory '{2}' attribute.",
                        model.Name,
                        lifestyleType,
                        attribute
                    )
