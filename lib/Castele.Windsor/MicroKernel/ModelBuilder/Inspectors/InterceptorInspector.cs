...
namespace Castle.MicroKernel.ModelBuilder.Inspectors
{
    ...
    public class InterceptorInspector : IContributeComponentModelConstruction
    {
        ...
        protected virtual void AddInterceptor(
        ...
        protected virtual void CollectHook(IConfiguration interceptors, ProxyOptions options)
        {
            ...
            if (hookComponent == null)
            {
                throw new Exception(
                    String.Format(
                        "The value for the hook must be a reference to a component (Currently {0})",
                        hook
                    )
                );
            }
        ...
        protected virtual void CollectSelector(IConfiguration interceptors, ProxyOptions options)
        {
            ...
            if (selectorComponent == null)
            {
                throw new Exception(
                    String.Format(
                        "The value for the selector must be a reference to a component (Currently {0})",
                        selector
                    )
                );
            }
        ...
        private void CollectInterceptors(ComponentModel model, IConfiguration interceptors)
        {
            ...
            {
                var interceptorComponent = ReferenceExpressionUtil.ExtractComponentName(
                    interceptor.Value
                );
                if (interceptorComponent == null)
                {
                    throw new Exception(
                        String.Format(
                            "The value for the interceptor must be a reference to a component (Currently {0})",
                            interceptor.Value
                        )
                    );
                }
