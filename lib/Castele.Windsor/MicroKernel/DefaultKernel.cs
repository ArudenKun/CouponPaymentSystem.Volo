...
namespace Castle.MicroKernel
{
        ...
        IKernel, IKernelEvents, IKernelInternal
    {
        ...
        /// </summary>
        private readonly Dictionary<string, ISubSystem> subsystems = new Dictionary<
            string,
            ISubSystem
        ...
        public DefaultKernel(SerializationInfo info, StreamingContext context)
        {
            ...
            HandlerRegistered += (HandlerDelegate)
        ...
        public virtual IConfigurationStore ConfigurationStore
        {
            get
            {
                return GetSubSystem(SubSystemConstants.ConfigurationStoreKey)
        ...
        public virtual IKernel AddFacility(IFacility facility)
        {
            ...
            facility.Init(
                this,
                ConfigurationStore.GetFacilityConfiguration(facility.GetType().FullName)
            );

            return this;
        }

        public IKernel AddFacility<T>()
        ...
        /// <returns> </returns>
        public virtual ILifestyleManager CreateLifestyleManager(
        ...
        )
        {
            ...
            switch (type)
            {
                ...
                case LifestyleType.Bound:
                    manager = new ScopedLifestyleManager(
                        CreateScopeAccessorForBoundLifestyle(model)
                    );
                    break;
                ...
                case LifestyleType.Pooled:
                    ...
                    if (
                        model.ExtendedProperties.Contains(
                            ExtendedPropertiesConstants.Pool_InitialPoolSize
                        )
                    )
                    {
                        initial = (int)
                    ...
                    if (
                        model.ExtendedProperties.Contains(
                            ExtendedPropertiesConstants.Pool_MaxPoolSize
                        )
                    )
                    {
                        maxSize = (int)
                            model.ExtendedProperties[ExtendedPropertiesConstants.Pool_MaxPoolSize];
                    }
        ...
        {
            var selector =
                (Func<IHandler[], IHandler>)model.ExtendedProperties[Constants.ScopeRootSelector];
            if (selector == null)
            {
                throw new ComponentRegistrationException(
                    string.Format(
                        "Component {0} has lifestyle {1} but it does not specify mandatory 'scopeRootSelector'.",
                        model.Name,
                        LifestyleType.Bound
                    )
                );
        ...
        public virtual IComponentActivator CreateComponentActivator(ComponentModel model)
        {
            ...
            {
                activator = new DefaultComponentActivator(
                    model,
                    this,
            ...
            else
            {
                try
                {
                    activator = model.CustomComponentActivator.CreateInstance<IComponentActivator>(
                        new object[]
                        {
                            model,
                            this,
        ...
        protected CreationContext CreateCreationContext(
            IHandler handler,
            Type requestedType,
        ...
        private object ResolveComponent(
            IHandler handler,
            Type service,
            Arguments additionalArguments,
            IReleasePolicy policy,
            bool ignoreParentContext
        )
        {
            Debug.Assert(handler != null, "handler != null");
            var parent = currentCreationContext;
            var context = CreateCreationContext(
                handler,
                service,
                additionalArguments,
                ignoreParentContext ? null : parent,
                policy
            );
        ...
        protected virtual IHandler WrapParentHandler(IHandler parentHandler)
        {
            ...
            var handler = new ParentHandlerWrapper(
                parentHandler,
                Parent.Resolver,
                Parent.ReleasePolicy
            );
            handler.Init(this);
            return handler;
        }
