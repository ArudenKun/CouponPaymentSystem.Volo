...
namespace Castle.MicroKernel.Handlers
{
    ...
    public class GenericHandlerTypeMismatchException : HandlerException
    {
        ...
        public GenericHandlerTypeMismatchException(
            IEnumerable<Type> argumentsUsed,
            ComponentModel componentModel,
            DefaultGenericHandler handler
        )
            : base(
                BuildMessage(
                    argumentsUsed.Select(a => a.FullName).ToArray(),
                    componentModel,
                    handler
                ),
        ...
        )
        {
            ...
            return message
                + string.Format(
                    "this is likely a bug in the {0} used ({1})",
