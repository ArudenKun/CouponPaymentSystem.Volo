...
namespace Castle.MicroKernel.Handlers
{
            ...
            IDisposable
    {
        ...
        public void ObtainDependencyDetails(IDependencyInspector inspector)
        {
            ...
            inspector.Inspect(
                this,
                missing != null ? missing.ToArray() : new DependencyModel[0],
                Kernel
            );
        }
        ...
        public object TryResolve(CreationContext context)
        {
            ...
            catch (DependencyResolverException e)
            {
                ...
                Kernel.Logger.LogWarning(
                    e,
                    "Tried to resolve component {Name} but failed due to exception. Ignoring.",
                    ComponentModel.Name
                );
                return null;
            }
            catch (HandlerException e)
            {
                ...
                Kernel.Logger.LogWarning(
                    e,
                    "Tried to resolve component {0} but failed due to exception. Ignoring.",
                    ComponentModel.Name
                );
                return null;
            }
        ...
        )
        {
            ...
            return HasCustomParameter(dependency.DependencyKey)
                || HasCustomParameter(dependency.TargetItemType);
        }
        ...
        private bool CanProvideDependenciesDynamically(CreationContext context)
        {
            return context.HasAdditionalArguments
                || kernel.HasComponent(typeof(ILazyComponentLoader));
        }
        ...
        private bool AllRequiredDependenciesResolvable()
        {
            ...
            var constructorDependencies = dependencies
            ...
            var ctorsWithMissingDependenciesCount = constructorDependencies
        ...
        {
            return Kernel.Resolver.CanResolve(
                CreationContext.ForDependencyInspection(this),
                this,
                model,
                dependency
            );
