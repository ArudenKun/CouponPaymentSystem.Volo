...
namespace Castle.MicroKernel.Handlers
{
    ...
    public class DefaultHandlerFactory : IHandlerFactory
    {
        ...
        private IGenericImplementationMatchingStrategy GenericImplementationMatchingStrategy(
            ComponentModel model
        )
        {
            return (IGenericImplementationMatchingStrategy)
                model.ExtendedProperties[Constants.GenericImplementationMatchingStrategy];
        }

        private IGenericServiceStrategy GenericServiceStrategy(ComponentModel model)
        {
            return (IGenericServiceStrategy)
                model.ExtendedProperties[Constants.GenericServiceStrategy];
        }
    }
}
