...
namespace Castle.Facilities.TypedFactory
{
    ...
    public class TypedFactoryFacility : AbstractFacility
    {
        ...
        [Obsolete(
            "This method is obsolete. Use AsFactory() extension method on fluent registration API instead."
        )]
        ...
        public void AddTypedFactoryEntry(FactoryEntry entry)
        {
            var model = new ComponentModel(
                ...
                typeof(Empty),
                new Arguments { { "typed.fac.entry", entry } }
            )
            {
        ...
        {
            Kernel.Register(
                Component.For<TypedFactoryInterceptor>().NamedAutomatically(InterceptorKey),
                Component
        ...
        {
            Kernel.Register(
                Component.For<FactoryInterceptor>().NamedAutomatically("typed.fac.interceptor")
            );
        ...
        private void LegacyRegisterFactory(
            string id,
            Type factoryType,
            string creation,
            string destruction
