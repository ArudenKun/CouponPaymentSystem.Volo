...
namespace Castle.Facilities.TypedFactory
{
    ...
    public static class TypedFactoryRegistrationExtensions
    {
            ...
            where TFactoryInterface : class
        {
            ...
            var classServices = registration
                .Services.TakeWhile(s => s.GetTypeInfo().IsClass)
                .ToArray();
        ...
        {
            var selectorReference = GetSelectorReference(
                configuration,
                defaultComponentSelectorKey,
                typeof(TFactory)
            );
        ...
        {
            return registration
                .Interceptors(new InterceptorReference(TypedFactoryFacility.InterceptorKey))
                .Last;
        }
            ...
            where TDelegate : class
        {
            if (HasOutArguments(delegateType))
            {
                throw new ComponentRegistrationException(
                    string.Format(
                        "Delegate type {0} can not be used as typed factory because it has 'out' arguments.",
                        delegateType
                    )
                );
            }
            ...
            if (invoke == null)
            {
                throw new ComponentRegistrationException(
                    string.Format(
                        "Delegate type {0} can not be used as typed factory because it has void return type.",
                        delegateType
                    )
                );
            }
            var settings = new TypedFactoryConfiguration(
                TypedFactoryFacility.DefaultDelegateSelectorKey,
                typeof(TDelegate)
            );
            if (configuration != null)
            {
        ...
        private static ComponentRegistration<TFactoryInterface> RegisterInterfaceBasedFactory<TFactoryInterface>(
            ComponentRegistration<TFactoryInterface> registration,
            Action<TypedFactoryConfiguration> configuration
        )
            where TFactoryInterface : class
        {
            foreach (var serviceType in registration.Services)
            {
                ...
                if (HasOutArguments(serviceType))
                {
                    throw new ComponentRegistrationException(
                        string.Format(
                            "Type {0} can not be used as typed factory because it has methods with 'out' arguments.",
                            serviceType
                        )
                    );
                }
            ...
            return AttachConfiguration(
                componentRegistration,
                configuration,
                TypedFactoryFacility.DefaultInterfaceSelectorKey
            );
        }
    }
}
