        ...
        public IRegistration Load(string name, Type service, Arguments arguments)
        {
            ...
            return Component
                ...
                .LifeStyle.Transient.Interceptors(
                    new InterceptorReference(TypedFactoryFacility.InterceptorKey)
                )
                .Last.Activator<DelegateFactoryActivator>()
                .DynamicParameters(
                    (k, args) =>
                    {
                        var selector = k.Resolve<ITypedFactoryComponentSelector>(
