...
namespace Castle.Facilities.TypedFactory
{
    ...
    public class TypedFactoryComponentResolver
    {
        ...
        public TypedFactoryComponentResolver(
            string componentName,
            Type componentType,
            Arguments additionalArguments,
            bool fallbackToResolveByTypeIfNameNotFound,
            Type actualSelectorType
        )
        {
        ...
        public virtual object Resolve(IKernelInternal kernel, IReleasePolicy scope)
        {
            if (LoadByName(kernel))
            {
                ...
                {
                    if (
                        actualSelectorType == typeof(DefaultDelegateComponentSelector)
                        && fallbackToResolveByTypeIfNameNotFound == false
                    )
                    {
                        e.Data["breakingChangeId"] =
                            "typedFactoryFallbackToResolveByTypeIfNameNotFound";
        ...
        private bool LoadByName(IKernelInternal kernel)
        {
            ...
            return fallbackToResolveByTypeIfNameNotFound == false
                || kernel.LoadHandlerByName(componentName, componentType, additionalArguments)
                    != null;
        }
    }
}
