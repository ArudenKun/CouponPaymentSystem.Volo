    ...
    public class TypedFactoryConfiguration
    {
        ...
        public TypedFactoryConfiguration(string defaultComponentSelectorKey, Type factoryType)
        {
            ...
            if (attributes.Length > 0)
            {
                ...
                {
                    SelectedWith(
                        defaults.SelectorType.CreateInstance<ITypedFactoryComponentSelector>()
                    );
                }
            }
        }
        ...
        {
            selectorReference = new ComponentReference<ITypedFactoryComponentSelector>(
                selectorComponentName
            );
        }

        public void SelectedWith<TSelectorComponent>()
            where TSelectorComponent : ITypedFactoryComponentSelector
        {
