...
namespace Castle.Windsor.Configuration.Interpreters.XmlProcessor.ElementProcessors
{
    ...
    public class DefaultTextNodeProcessor : AbstractXmlNodeProcessor
    {
        ...
        ///   spaces are trimmed
        /// </summary>
        private static readonly Regex PropertyValidationRegExp = new Regex(
            @"(\#!?\{\s*((?:\w|\.)+)\s*\})",
                ...
                else if (IsRequiredProperty(propRef))
                {
...
#endif
                    {
                        throw new XmlProcessorException(
                            String.Format("Required configuration property {0} not found", propKey)
                        );
                    }
                }
        ...
        private void MoveAttributes(XmlElement targetElement, XmlElement srcElement)
        {
            ...
            {
                var importedAttr =
                    ImportNode(targetElement, srcElement.Attributes[i]) as XmlAttribute;
