    ...
    public class DuplicatedDependenciesDiagnostic : IDuplicatedDependenciesDiagnostic
    {
        ...
        public string GetDetails(DependencyDuplicate duplicates)
        {
            ...
            switch (duplicates.Reason)
            {
                ...
                case DependencyDuplicationReason.Reference:
                    details.Append(
                        ", they both reference the same component "
        ...
        public Tuple<IHandler, DependencyDuplicate[]>[] Inspect()
        {
            ...
            foreach (var handler in allHandlers)
            {
                ...
                {
                    result.Add(
                        new Tuple<IHandler, DependencyDuplicate[]>(handler, duplicateDependencies)
                    );
                }
            }
        ...
        private void CollectDuplicatesBetween(
            DependencyModel[] array,
        ...
        private void CollectDuplicatesBetweenConstructorParameters(
            ConstructorCandidateCollection constructors,
            ICollection<DependencyDuplicate> duplicates
        )
        {
        ...
        private void CollectDuplicatesBetweenProperties(
            DependencyModel[] properties,
            ICollection<DependencyDuplicate> duplicates
        )
        {
        ...
        private void CollectDuplicatesBetweenPropertiesAndConstructors(
            ConstructorCandidateCollection constructors,
            DependencyModel[] properties,
            ICollection<DependencyDuplicate> duplicates
        )
        ...
        private DependencyDuplicate[] FindDuplicateDependenciesFor(IHandler handler)
        {
            ...
            var properties = handler
                .ComponentModel.Properties.Select(p => p.Dependency)
        ...
        private DependencyDuplicationReason IsDuplicate(DependencyModel foo, DependencyModel bar)
        {
            if (foo.ReferencedComponentName != null || bar.ReferencedComponentName != null)
            {
                if (
                    string.Equals(
                        foo.ReferencedComponentName,
                        bar.ReferencedComponentName,
        ...
        private static void Describe(StringBuilder details, DependencyModel dependency)
        {
            ...
            details.Append(
                dependency.TargetItemType.ToCSharpString() + " " + dependency.DependencyKey
            );
        }
    }
}
