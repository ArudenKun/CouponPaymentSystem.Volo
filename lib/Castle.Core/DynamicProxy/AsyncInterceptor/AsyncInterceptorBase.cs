...
public abstract class AsyncInterceptorBase : IAsyncInterceptor
{
    ...
    private static readonly ConcurrentDictionary<
        Type,
        GenericSynchronousHandler
    > GenericSynchronousHandlers = new() { [typeof(void)] = InterceptSynchronousVoid };

    private delegate void GenericSynchronousHandler(
        AsyncInterceptorBase me,
    ...
    public void InterceptSynchronous(IInvocation invocation)
    {
        ...
        GenericSynchronousHandler handler = GenericSynchronousHandlers.GetOrAdd(
            returnType,
            CreateHandler
        );
    ...
    private static void InterceptSynchronousResult<TResult>(
        AsyncInterceptorBase me,
        IInvocation invocation
    )
    {
        Task<TResult> task = me.InterceptAsync(
            invocation,
            invocation.CaptureProceedInfo(),
            ProceedSynchronous<TResult>
        );
    ...
    private static Task ProceedSynchronous(
        IInvocation invocation,
        IInvocationProceedInfo proceedInfo
    )
    {
    ...
    [SuppressMessage(
        ...
        Justification = "The name explicitly says Asynchronous."
    )]
    private static async Task ProceedAsynchronous(
        IInvocation invocation,
        IInvocationProceedInfo proceedInfo
    )
    {
