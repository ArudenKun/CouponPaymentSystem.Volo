...
internal class MethodWithInvocationGenerator : MethodGenerator
{
    ...
    )
    {
        ...
        else
        {
            ...
            @class.ClassConstructor.CodeBuilder.AddStatement(
                new AssignStatement(proxiedMethodToken, new MethodTokenExpression(MethodToOverride))
            );
        ...
        {
            EmitLoadGenericMethodArguments(
                emitter,
                MethodToOverride.MakeGenericMethod(genericArguments),
                invocationLocal
            );
    ...
    private void EmitLoadGenericMethodArguments(
