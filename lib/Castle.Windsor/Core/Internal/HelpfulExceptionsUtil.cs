    ...
    public static class HelpfulExceptionsUtil
    {
        public static Exception TrackInstanceCalledMultipleTimes(object instance, Burden burden)
        {
            ...
            else
            {
                ...
                message.Append(
                    "In most cases it is advised for the activator not to be handling reuse of the instances, but to chose a lifestyle that does that appropriately."
                );
            }
        ...
        private static bool IsUsingFactoryMethod(ComponentModel componentModel)
        {
            return componentModel.CustomComponentActivator != null
                ...
                && componentModel.CustomComponentActivator.GetGenericTypeDefinition()
                    == typeof(FactoryMethodActivator<>);
        }
    }
}
