...
namespace Castle.Core.Internal
{
    ...
    {
        private static readonly TypeByInheritanceDepthMostSpecificFirstComparer comparer =
            new TypeByInheritanceDepthMostSpecificFirstComparer();

        public static void AddService(
            IList<Type> existingServices,
            HashSet<Type> lookup,
            Type newService
        )
        {
            ...
            if (newService.GetTypeInfo().IsClass == false)
            {
                throw new ArgumentException(
                    string.Format(
                        "Type {0} is not a class nor an interface, and those are the only values allowed.",
                        newService
                    )
                );
            }
