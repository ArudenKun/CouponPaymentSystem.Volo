...
namespace Castle.Facilities.TypedFactory.Internal
{
    ...
    public class TypedFactoryInterceptor : IInterceptor, IOnBehalfAware, IDisposable
    {
        ...
        public TypedFactoryInterceptor(
        ...
        {
            methods =
                (IDictionary<MethodInfo, FactoryMethod>)
                    target.ExtendedProperties[TypedFactoryFacility.FactoryMapCacheKey];
            if (methods == null)
            {
                throw new ArgumentException(
                    string.Format(
                        "Component {0} is not a typed factory. {1} only works with typed factories.",
                        target.Name,
                        GetType().Name
                    )
                );
        ...
        private void Resolve(IInvocation invocation)
        {
            ...
            var component = ComponentSelector.SelectComponent(
                invocation.Method,
