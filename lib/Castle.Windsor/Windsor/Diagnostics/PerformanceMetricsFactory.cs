...
namespace Castle.Windsor.Diagnostics
{
    ...
    public class PerformanceMetricsFactory : IPerformanceMetricsFactory
    {
        ...
        private const string InstanesTrackedByTheReleasePolicyCounterName =
            "Instances tracked by the release policy";
        ...
        public ITrackedComponentsPerformanceCounter CreateInstancesTrackedByReleasePolicyCounter(
            string name
        )
        {
                ...
                if (PerformanceCounterCategory.Exists(CastleWindsorCategoryName))
                {
                    Debug.Assert(
                        PerformanceCounterCategory
                            .GetCategories()
                            .Count(c => c.CategoryName == CastleWindsorCategoryName) == 1
                    );
                }
                else
                {
