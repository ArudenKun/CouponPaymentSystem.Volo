...
namespace Castle.Windsor.Installer
{
    ...
    public class DefaultComponentInstaller : IComponentsInstaller
    {
        ...
        {
            var converter =
                container.Kernel.GetSubSystem(SubSystemConstants.ConversionManagerKey)
        ...
        protected virtual void SetUpInstallers(
            IConfiguration[] installers,
        ...
        private void AddInstaller(
            IConfiguration installer,
            Dictionary<Type, IWindsorInstaller> cache,
        ...
        private void GetAssemblyInstallers(
            Dictionary<Type, IWindsorInstaller> cache,
            Assembly assembly
        )
        {
        ...
        protected virtual void SetUpFacilities(
        ...
        private void AssertImplementsService(IConfiguration id, Type service, Type implementation)
        {
            ...
            if (!service.IsAssignableFrom(implementation))
            {
                var message = string.Format(
                    "Could not set up component '{0}'. Type '{1}' does not implement service '{2}'",
                    id.Attributes["id"],
        ...
        )
        {
            foreach (var component in configurations)
            {
                ...
                container.Register(
                    Component.For(services).ImplementedBy(implementation).Named(name)
                );
            }
        }
        ...
        private void CollectAdditionalServices(
            ...
            foreach (var forwardedType in forwardedTypes.Children)
            {
                ...
                catch (ConverterException e)
                {
                    throw new ComponentRegistrationException(
                        string.Format(
                            "Component {0} defines invalid forwarded type.",
        ...
        private static void SetUpChildContainers(
            IConfiguration[] configurations,
            IWindsorContainer parentContainer
        )
        {
