    ...
    public class XmlInterpreter : AbstractInterpreter
    {
            ...
            foreach (XmlNode node in section)
            {
                if (XmlConfigurationDeserializer.IsTextNode(node))
                {
                    throw new ConfigurationProcessingException(
                        String.Format("{0} cannot contain text nodes", node.Name)
                    );
                }
        ...
        private static void AssertNodeName(XmlNode node, IEquatable<string> expectedName)
        {
            ...
            var message = String.Format(
                "Unexpected node under '{0}': Expected '{1}' but found '{2}'",
                expectedName,
                expectedName,
                node.Name
            );
        ...
        private static void DeserializeComponents(
            XmlNodeList nodes,
            IConfigurationStore store,
            IConversionManager converter
        )
        ...
        private static void DeserializeElement(
            XmlNode node,
            IConfigurationStore store,
            IConversionManager converter
        )
        ...
        private static void DeserializeFacilities(
            XmlNodeList nodes,
            IConfigurationStore store,
            IConversionManager converter
        )
        ...
        private static void DeserializeFacility(
            XmlNode node,
            IConfigurationStore store,
            IConversionManager converter
        )
        ...
        )
        {
            ...
            {
                var message = String.Format(
                    "{0} elements expects required non blank attribute {1}",
                    configuration.Name,
                    attributeName
                );
