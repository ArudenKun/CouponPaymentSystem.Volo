...
namespace Castle.Windsor.Configuration.Interpreters.XmlProcessor
{
    ...
    public class DefaultXmlProcessorEngine : IXmlProcessorEngine
    {
        ...
        private readonly IDictionary<
            XmlNodeType,
            IDictionary<string, IXmlNodeProcessor>
        > nodeProcessors = new Dictionary<XmlNodeType, IDictionary<string, IXmlNodeProcessor>>();
        ...
        public DefaultXmlProcessorEngine(
        ...
        public void AddNodeProcessor(Type type)
        {
            ...
            else
            {
                throw new XmlProcessorException(
                    "{0} does not implement {1} interface",
                    type.FullName,
        ...
        private void RegisterProcessor(XmlNodeType type, IXmlNodeProcessor processor)
        {
            ...
            {
                throw new XmlProcessorException(
                    "There is already a processor register for {0} with name {1} ",
                    type,
                    processor.Name
                );
            }
