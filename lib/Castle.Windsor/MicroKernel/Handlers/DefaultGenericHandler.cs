...
namespace Castle.MicroKernel.Handlers
{
    ...
    public class DefaultGenericHandler : AbstractHandler
    {
        ...
        private readonly SimpleThreadSafeDictionary<Type, IHandler> type2SubHandler =
            new SimpleThreadSafeDictionary<Type, IHandler>();
        ...
        public override bool Supports(Type service)
        {
            ...
            if (
                ...
                && service.GetTypeInfo().IsGenericTypeDefinition == false
            )
            {
        ...
        public override bool SupportsAssignable(Type service)
        {
            ...
            if (
                service.GetTypeInfo().IsGenericType == false
            ...
            return ComponentModel.Services.Any(s =>
                SupportsAssignable(service, s, serviceArguments)
            );
        }
        ...
        protected virtual Type[] AdaptServices(Type closedImplementationType, Type requestedType)
        {
            ...
            if (
                ...
                && requestedType.GetTypeInfo().IsGenericType
        ...
        protected virtual IHandler BuildSubHandler(
        ...
        {
            var added = false;
            var handler = type2SubHandler.GetOrAdd(
                genericType,
                t =>
                {
                    added = true;
        ...
        protected override void InitDependencies()
        {
            ...
            var activator =
                Kernel.CreateComponentActivator(ComponentModel) as IDependencyAwareActivator;
        ...
        {
            if (
                modelService.GetTypeInfo().IsGenericTypeDefinition == false
            ...
            if (
                ...
                && ServiceStrategy.Supports(modelServiceClosed, ComponentModel) == false
            )
            {
                return false;
            }
        ...
        public IHandler ConvertToClosedGenericHandler(
            Type service,
            CreationContext openGenericContext
        )
        {
        ...
        private Type GetClosedImplementationType(CreationContext context, bool instanceRequired)
        {
            ...
            catch (ArgumentException e)
            {
                ...
                // 2.
                var invalidArguments = genericArguments
                    .Where(a => a.IsPointer || a.IsByRef || a == typeof(void))
                    .Select(t => t.FullName)
                    .ToArray();
                ...
                throw new GenericHandlerTypeMismatchException(
                    genericArguments,
                    ComponentModel,
                    this
                );
        ...
        private Type[] GetGenericArguments(CreationContext context)
        {
            ...
            return implementationMatchingStrategy.GetGenericArguments(ComponentModel, context)
                ?? context.GenericArguments;
        }

        private static int AdaptClassServices(
            Type closedImplementationType,
            List<Type> closedServices,
            Type[] openServices
        )
        {
            var index = 0;
            ...
            while (index < openServices.Length && openServices[index].GetTypeInfo().IsClass)
            {
                ...
                if (service.GetTypeInfo().IsGenericTypeDefinition)
                {
                    ...
                    else
                    {
                        ...
                        Debug.Fail(
                            string.Format(
                                "Could not find mapping for interface {0} on implementation type {1}",
                                service,
                                closedImplementationType
                            )
                        );
        ...
        private static void AdaptInterfaceServices(
            Type closedImplementationType,
            List<Type> closedServices,
            Type[] openServices,
            int index
            ...
            while (index < openServices.Length)
            {
                ...
                if (service.GetTypeInfo().IsGenericTypeDefinition)
                {
                    EnsureInterfaceMappingInitialized(
                        closedImplementationType,
                        ref genericDefinitionToInterface
                    );
                    Type closed;
        ...
        private static void EnsureClassMappingInitialized(
            Type closedImplementationType,
        ...
        private static void EnsureInterfaceMappingInitialized(
            Type closedImplementationType,
