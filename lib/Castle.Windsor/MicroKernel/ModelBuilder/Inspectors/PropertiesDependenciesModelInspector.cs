...
namespace Castle.MicroKernel.ModelBuilder.Inspectors
{
    ...
    public class PropertiesDependenciesModelInspector : IContributeComponentModelConstruction
    {
        ...
        protected virtual void InspectProperties(ComponentModel model)
        {
            ...
            {
                model.InspectionBehavior = GetInspectionBehaviorFromTheConfiguration(
                    model.Configuration
                );
            }
            ...
            {
                foreach (
                    var filter in filters.Concat(
                        new[] { StandardPropertyFilters.Create(PropertyFilter.Default) }
                    )
                )
                {
        ...
        private PropertiesInspectionBehavior GetInspectionBehaviorFromTheConfiguration(
        ...
        {
            return IsSettable(property)
                && HasParameters(property) == false
                && HasDoNotWireAttribute(property) == false;
        }
    }
}
