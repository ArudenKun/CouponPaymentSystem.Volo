...
namespace Castle.MicroKernel.ModelBuilder.Inspectors
{
    ...
    public abstract class MethodMetaInspector : IContributeComponentModelConstruction
    {
                    ...
                    if (methods.Count == 0)
                    {
                        var message = String.Format(
                            "The class {0} has tried to expose configuration for "
                                + "a method named {1} which could not be found.",
        ...
        protected virtual void ProcessMeta(
        ...
        private void AssertNameIsNotNull(string name, ComponentModel model)
        {
            ...
            {
                var message = String.Format(
                    "The configuration nodes within 'methods' "
        ...
        private Type[] ConvertSignature(string signature)
        {
            ...
            foreach (var param in parameters)
            {
                ...
                {
                    var message = String.Format(
                        "The signature {0} contains an entry type {1} "
        ...
        private IList<MethodInfo> GetMethods(Type implementation, String name, String signature)
        {
            ...
            {
                var methodInfo = implementation.GetMethod(
                    name,
                    AllMethods,
                    null,
