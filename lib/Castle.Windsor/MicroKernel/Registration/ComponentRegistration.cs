...
namespace Castle.MicroKernel.Registration
{
        ...
        where TService : class
    {
        private readonly List<IComponentModelDescriptor> descriptors =
            new List<IComponentModelDescriptor>();
        ...
        /// </summary>
        /// <returns> </returns>
        public ComponentRegistration<TService> Activator<TActivator>()
            where TActivator : IComponentActivator
        {
            return AddAttributeDescriptor(
                "componentActivatorType",
                typeof(TActivator).AssemblyQualifiedName
            );
        }
        ...
        /// </remarks>
        public ComponentRegistration<TService> DependsOn(
            DynamicParametersWithContextResolveDelegate resolve
        )
        {
        ...
        /// <param name = "resolve"> The delegate used for providing dynamic parameters. </param>
        /// <returns> </returns>
        public ComponentRegistration<TService> DynamicParameters(
            DynamicParametersResolveDelegate resolve
        )
        {
            return DynamicParameters((k, c, d) => resolve(k, d));
        }
        ...
        public ComponentRegistration<TService> ExtendedProperties(object anonymous)
        {
            return AddDescriptor(
                new ExtendedPropertiesDescriptor(new ReflectionBasedDictionaryAdapter(anonymous))
            );
        }
        ...
        {
            return Forward(
                new[] { typeof(TService2), typeof(TService3), typeof(TService4), typeof(TService5) }
            );
        }
        ...
        /// <returns> </returns>
        public ComponentRegistration<TService> ImplementedBy<TImpl>()
        ...
        )
        {
            if (implementation != null && implementation != typeof(LateBoundComponent))
            {
                var message = String.Format(
                    "This component has already been assigned implementation {0}",
            ...
            if (genericImplementationMatchingStrategy != null)
            {
                ExtendedProperties(
                    Property
                        .ForKey(Constants.GenericImplementationMatchingStrategy)
            ...
            {
                ExtendedProperties(
                    Property.ForKey(Constants.GenericServiceStrategy).Eq(genericServiceStrategy)
                );
            }
            return this;
        }
        ...
        /// </summary>
        /// <returns> </returns>
        public ComponentRegistration<TService> Interceptors<TInterceptor>()
            where TInterceptor : IInterceptor
        {
            return AddDescriptor(
                new InterceptorDescriptor(new[] { new InterceptorReference(typeof(TInterceptor)) })
            );
        }
        ...
        /// <returns> </returns>
        public ComponentRegistration<TService> LifestyleBoundTo(
            Func<IHandler[], IHandler> scopeRootBinder
        )
        {
        ...
        /// </summary>
        /// <returns> </returns>
        public ComponentRegistration<TService> LifestylePooled(
        ...
        public ComponentRegistration<TService> Named(String name)
        {
            ...
            {
                var message = String.Format(
                    "This component has already been assigned name '{0}'",
                    this.name.Name
                );
                throw new ComponentRegistrationException(message);
            }
        ...
        public ComponentRegistration<TService> OnCreate(params Action<TService>[] actions)
        {
            ...
            {
                return OnCreate(
                    actions.ConvertAll(a => new LifecycleActionDelegate<TService>((_, o) => a(o)))
                );
            }
            return this;
        }
        ...
        /// </summary>
        /// <param name = "actions"> A set of actions to be executed right after the component is created and before it's returned from the container. </param>
        public ComponentRegistration<TService> OnCreate(
            params LifecycleActionDelegate<TService>[] actions
        )
        {
            if (actions != null && actions.Length != 0)
            {
        ...
        public ComponentRegistration<TService> OnDestroy(
            params LifecycleActionDelegate<TService>[] actions
        )
        {
            if (actions != null && actions.Length != 0)
        ...
        public ComponentRegistration<TService> UsingFactory<TFactory, TServiceImpl>(
            Func<TFactory, TServiceImpl> factory
        )
            ...
            where TImpl : TService
        {
            ...
            {
                ExtendedProperties(
                    Property.ForKey("factory.managedExternally").Eq(managedExternally)
                );
            }
            ...
            if (
                implementation == null
                && (
                    potentialServices.First().GetTypeInfo().IsClass == false
                    || potentialServices.First().GetTypeInfo().IsSealed == false
                )
            )
            {
        ...
        void IRegistration.Register(IKernelInternal kernel)
        {
            ...
            {
                kernel.Logger.LogInformation("Skipping registration of " + componentModel.Name);
                return;
            }
        ...
        public ComponentRegistration<TService> IsDefault(Predicate<Type> serviceFilter)
        {
            ...
            var properties = new Property(
                Constants.DefaultComponentForServiceFilter,
                serviceFilter
            );
        ...
        public ComponentRegistration<TService> IsFallback(Predicate<Type> serviceFilter)
        {
            ...
            var properties = new Property(
                Constants.FallbackComponentForServiceFilter,
                serviceFilter
            );
            return ExtendedProperties(properties);
        }
        ...
        public ComponentRegistration<TService> PropertiesIgnore(
        ...
        public ComponentRegistration<TService> PropertiesRequire(
            Func<PropertyInfo, bool> propertySelector
        )
        {
        ...
        /// will never try to set it. </param>
        public ComponentRegistration<TService> PropertiesIgnore(
            Func<ComponentModel, PropertyInfo, bool> propertySelector
        )
        {
            return AddDescriptor(
                new DelegatingModelDescriptor(
                    builder: (k, c) =>
                    {
                        var filters = StandardPropertyFilters.GetPropertyFilters(
                            c,
