...
namespace Castle.MicroKernel.Resolvers
{
    ...
    {
        private readonly IList<ISubDependencyResolver> subResolvers =
            new List<ISubDependencyResolver>();
        ...
        )
        {
            ...
            if (
                CanResolveFromContextHandlerResolver(
                    context,
                    contextHandlerResolver,
                    model,
                    dependency
                )
            )
        ...
        protected virtual bool CanResolveFromKernel(
            CreationContext context,
            ComponentModel model,
            DependencyModel dependency
        )
        {
            if (dependency.ReferencedComponentName != null)
            {
                ...
                return HasComponentInValidState(
        ...
        protected virtual CreationContext RebuildContextForParameter(
        ...
        protected virtual object ResolveFromKernel(
            CreationContext context,
            ComponentModel model,
            DependencyModel dependency
        )
        ...
        private bool CanResolveFromContext(
            CreationContext context,
            ISubDependencyResolver contextHandlerResolver,
            ComponentModel model,
            DependencyModel dependency
        ...
        {
            return context != null
                && context.CanResolve(context, contextHandlerResolver, model, dependency);
        }

        private bool CanResolveFromContextHandlerResolver(
            CreationContext context,
            ISubDependencyResolver contextHandlerResolver,
            ComponentModel model,
            DependencyModel dependency
        )
        {
            return contextHandlerResolver != null
                && contextHandlerResolver.CanResolve(
                    context,
                    contextHandlerResolver,
                    model,
                    dependency
        ...
        private bool CanResolveFromHandler(
            CreationContext context,
            ISubDependencyResolver contextHandlerResolver,
            ComponentModel model,
            DependencyModel dependency
        ...
        private bool CanResolveFromSubResolvers(
            CreationContext context,
            ISubDependencyResolver contextHandlerResolver,
            ComponentModel model,
            DependencyModel dependency
        ...
        {
            return subResolvers.Count > 0
                && subResolvers.Any(s =>
                    s.CanResolve(context, contextHandlerResolver, model, dependency)
                );
        }

        private bool HasAnyComponentInValidState(
            Type service,
            DependencyModel dependency,
            CreationContext context
        )
        {
            ...
            {
                firstHandler = kernel.LoadHandlerByType(
                    dependency.DependencyKey,
                    service,
            ...
            var nonResolvingHandlers = handlers
                .Where(handler => handler.IsBeingResolvedInContext(context) == false)
                .ToList();
            RebuildOpenGenericHandlersWithClosedGenericSubHandlers(
                service,
                context,
                nonResolvingHandlers
            );
            return nonResolvingHandlers.Any(handler => IsHandlerInValidState(handler));
        }

        private void RebuildOpenGenericHandlersWithClosedGenericSubHandlers(
            Type service,
            CreationContext context,
        ...
        )
        {
            if (context.RequestedType != null && service.GetTypeInfo().IsGenericType)
            {
                ...
                var closedGenericSubHandlers = genericHandlers
                    .Select(x => x.ConvertToClosedGenericHandler(service, openGenericContext))
                    .ToList();
        ...
        private bool HasComponentInValidState(
            string key,
            DependencyModel dependency,
            CreationContext context
        )
        {
            ...
            if (context != null && context.IsResolving)
            {
                handler = kernel.LoadHandlerByName(
                    key,
        ...
        )
        {
            ...
            {
                value = contextHandlerResolver.Resolve(
                    context,
                    contextHandlerResolver,
                    model,
                    dependency
        ...
        private object ResolveFromKernelByName(
            CreationContext context,
            ComponentModel model,
            DependencyModel dependency
        )
        {
            var handler = kernel.LoadHandlerByName(
                dependency.ReferencedComponentName,
                dependency.TargetItemType,
                context.AdditionalArguments
            );
        ...
        private object ResolveFromKernelByType(
            CreationContext context,
            ComponentModel model,
            DependencyModel dependency
        )
        ...
        private object ResolveFromParameter(
            CreationContext context,
            ComponentModel model,
            DependencyModel dependency
        )
        {
            ...
            try
            {
                ...
                {
                    return converter.PerformConversion(
                        dependency.Parameter.ConfigValue,
                        dependency.TargetItemType
                    );
                }
            }
            catch (ConverterException e)
            {
                throw new DependencyResolverException(
                    string.Format(
                        "Could not convert parameter '{0}' to type '{1}'.",
                        dependency.Parameter.Name,
                        dependency.TargetItemType.Name
                    ),
                    e
                );
            }
        ...
        private bool TryGetHandlerFromKernel(
            DependencyModel dependency,
            CreationContext context,
        ...
        )
        {
            ...
            try
            {
                handler = kernel.LoadHandlerByType(
                    dependency.DependencyKey,
                    dependency.TargetItemType,
                    context.AdditionalArguments
                );
