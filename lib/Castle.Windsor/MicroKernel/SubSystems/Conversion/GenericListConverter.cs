...
namespace Castle.MicroKernel.SubSystems.Conversion
{
    ...
    public class GenericListConverter : AbstractTypeConverter
    {
            ...
            if (ReferenceExpressionUtil.IsReference(value))
            {
                ...
                if (handler == null)
                {
                    throw new ConverterException(
                        string.Format("Component '{0}' was not found in the container.", newValue)
                    );
                }

                return handler.Resolve(
                    Context.CurrentCreationContext ?? CreationContext.CreateEmpty()
                );
            }
        ...
        public override object PerformConversion(IConfiguration configuration, Type targetType)
        {
            ...
            var converterHelper = helperType.CreateInstance<IGenericCollectionConverterHelper>(
                this
            );
