...
public class EmbeddedResourceVirtualPathProvider : VirtualPathProvider, ITransientDependency
{
    ...
    public EmbeddedResourceVirtualPathProvider(IVirtualFileProvider virtualFileProvider)
    {
    ...
    )
    {
        ...
        foreach (string virtualPathDependency in pathDependencies)
        {
            ...
            if (
                GetResource(virtualPathDependency) != null
                || GetResources(virtualPathDependency).Any()
            )
            {
                foundAny = true;
            }
    ...
    public override VirtualDirectory GetDirectory(string virtualDir)
    {
        ...
        {
            return new ComponentsEmbeddedResourceVirtualDirectory(
                virtualDir,
    ...
    private IFileInfo GetResource(string virtualPath)
    {
    ...
    private IEnumerable<IFileInfo> GetResources(string virtualPath)
    {
        return _virtualFileProvider.GetDirectoryContents(
            VirtualPathUtility.ToAppRelative(virtualPath).RemovePreFix("~")
        );
    }
}
