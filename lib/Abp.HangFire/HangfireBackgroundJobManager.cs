...
internal class HangfireBackgroundJobManager : BackgroundWorkerBase, IBackgroundJobManager
{
    ...
    public override void Start()
    {
        ...
        if (
            _hangfireConfiguration.Server == null
            && _backgroundJobConfiguration.IsJobExecutionEnabled
        )
        {
    ...
    public virtual Task<string> EnqueueAsync<TJob, TArgs>(
        TArgs args,
        ...
        where TJob : IBackgroundJobBase<TArgs>
    {
        ...
        if (!delay.HasValue)
        {
            ...
            {
                jobUniqueIdentifier = _backgroundJobClient.Enqueue<TJob>(job =>
                    ((IBackgroundJob<TArgs>)job).Execute(args)
                );
            }
            else
            {
        ...
        else
        {
            if (typeof(IBackgroundJob<TArgs>).IsAssignableFrom(typeof(TJob)))
            {
                jobUniqueIdentifier = _backgroundJobClient.Schedule<TJob>(
            ...
            {
                jobUniqueIdentifier = _backgroundJobClient.Schedule<TJob>(
                    job => ((IAsyncBackgroundJob<TArgs>)job).ExecuteAsync(args),
                    delay.Value
                );
            }
        }
